...
def fetch_stocktwits_messages(ticker: str, limit: int = 30, timeout: float = 10.0) -> str:
    ...
    """
    stream_symbol = stocktwits_symbol(ticker)
    url = _API.format(ticker=stream_symbol)
    ...
    try:
        ...
        logger.warning("StockTwits fetch failed for %s: %s", stream_symbol, exc)
        return f"<stocktwits unavailable: {type(exc).__name__}>"

    messages = data.get("messages", []) if isinstance(data, dict) else []
    if not messages:
        return f"<no StockTwits messages found for ${stream_symbol}>"
